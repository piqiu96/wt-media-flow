...
class ClawWorkflow:
        ...
        try:
            ...
            for i, item in enumerate(items, 1):
                ...
                try:
                    ...
                    if existing:
                        logger.info(f"[{i}/{len(items)}] 跳过（已存在）: {vid} - {title[:40]}")
                        result["skipped"] += 1
                        continue
                    ...
                    logger.info(f"[{i}/{len(items)}] 入库成功: {vid} - {title[:40]} "
                                f"(id={video.id} 点赞={like_count} 收藏={collect_count})")
                    result["added"] += 1

                except Exception as e:
                    db.rollback()
                    logger.error(f"[{i}/{len(items)}] 入库失败: {vid} - {e}")
                    result["failed"] += 1
    ...
    def download_pending(self, category: str = None, limit: int = 200,
        ...
        try:
            ...
            if retry_failed:
                ...
                if failed_videos:
                    ...
                    logger.info(f"已重置 {len(ids)} 条 failed → pending")
        ...
        logger.info(f"待下载: {len(videos)} 条")
        for i, video in enumerate(videos, 1):
            vid = video.source_vid or str(video.id)
            logger.info(f"[{i}/{len(videos)}] vid={vid} {(video.title or '')[:40]}")
    ...
    def _download_video(self, video) -> str | None:
        ...
        if not video_url:
            logger.warning(f"vid={video.source_vid} 无远程 URL，跳过")
            return None
        ...
        if os.path.isfile(local_path):
            logger.info(f"本地已存在: {local_path}")
            return local_path
        ...
        def _do_download(url: str) -> str | None:
            try:
                ...
                logger.info(f"下载完成: {local_path} ({size_mb:.1f}MB)")
                return local_path
            except Exception as e:
        ...
        # URL 过期，重新拉取直链重试
        if result == "403":
            logger.warning(f"vid={vid} URL 已过期，刷新直链中...")
            try:
            ...
            if fresh_url:
                ...
                db2 = SessionLocal()
                try:
                    v = db2.query(Video).filter_by(id=video.id).first()
                    if v:
                ...
                logger.error(f"vid={vid} 无法获取新 URL")
                result = None
